...
public class LampQueueChecker9 extends SimProcess{
	public boolean zapalana = false;
	public boolean swieci = false;
	public boolean gasnie = false;
	
	public LampQueueChecker9(Model owner, String name, boolean showInTrace) {
	...
	public void lifeCycle() {
		...
		while(true){
			if(!model.Lamp9Queue.isEmpty()){
				car = model.Lamp9Queue.first();
				...
				if(car != null){
					model.Lamp9Queue.remove(car);
//					car.activateAfter(this);
				}

			}else{
				if(car != null && model.Lamp9Queue.isEmpty() && swieci){
					swieci = false;
					gasnie = true;
