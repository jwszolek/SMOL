...
public class LampQueueChecker6 extends SimProcess {
	public boolean zapalana = false;
	public boolean swieci = false;
	public boolean gasnie = false;
	
	public LampQueueChecker6(Model owner, String name, boolean showInTrace) {
	...
	public void lifeCycle() {
		...
		while(true){
			if(!model.Lamp6Queue.isEmpty()){
				car = model.Lamp6Queue.first();
				...
				if(car != null){
					model.Lamp6Queue.remove(car);
//					car.activateAfter(this);
				}

			}else{
				if(car != null && model.Lamp6Queue.isEmpty() && swieci){
					swieci = false;
					gasnie = true;
