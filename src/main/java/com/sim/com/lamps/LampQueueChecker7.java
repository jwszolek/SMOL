...
public class LampQueueChecker7 extends SimProcess{
	public boolean zapalana = false;
	public boolean swieci = false;
	public boolean gasnie = false;
	
	public LampQueueChecker7(Model owner, String name, boolean showInTrace) {
	...
	public void lifeCycle() {
		...
		while(true){
			if(!model.Lamp7Queue.isEmpty()){
				car = model.Lamp7Queue.first();
				...
				if(car != null){
					model.Lamp7Queue.remove(car);
//					car.activateAfter(this);
				}

			}else{
				if(car != null && model.Lamp7Queue.isEmpty() && swieci){
					swieci = false;
					gasnie = true;
