...
public class LampQueueChecker2 extends SimProcess{
	
	public boolean zapalana = false;
	public boolean swieci = false;
	public boolean gasnie = false;
	
	public LampQueueChecker2(Model owner, String name, boolean showInTrace) {
	...
	public void lifeCycle() {
		...
		while(true){
			if(!model.Lamp2Queue.isEmpty()){
				car = model.Lamp2Queue.first();
				...
				if (zapalana && car != null){
					...
					sendTraceNote(car.getName() +"2"+ " $Z_START");
					hold(new TimeSpan(2, TimeUnit.SECONDS));
					sendTraceNote(car.getName() +"2"+ " $Z_END");
					zapalana = false;
					swieci = true;
				}
				...
				if (swieci){
					...
					sendTraceNote(car.getName() +"2"+ " $S_START");
					hold(new TimeSpan(10, TimeUnit.SECONDS));
					sendTraceNote(car.getName() +"2"+ " $S_END");
				}
				
				
				if(car != null){
					model.Lamp2Queue.remove(car);
//					car.activateAfter(this);
				...
				if(model.Lamp1Queue.isEmpty() && swieci){
					...
					sendTraceNote(car.getName() +"2"+ " $G_START");
					hold(new TimeSpan(3, TimeUnit.SECONDS));
					sendTraceNote(car.getName() +"2"+ " $G_END");
					gasnie = false;
				}else{
